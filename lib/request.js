...
var url = require( 'url' );
var debug = require( 'debug' )( 'npm-list-package-dependents:request' );
...
*	Queries an endpoint for package depending upon a specified package.
*
...
function request( options, clbk ) {
	...
	opts.path += '/-/_view/dependedUpon?';
	opts.path += 'group_level=2';
	opts.path += '&';
	opts.path += 'start_key=["'+options.package+'"]';
	opts.path += '&';
	opts.path += 'end_key=["'+options.package+'",{}]';
